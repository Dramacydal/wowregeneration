...
namespace WoWRegeneration.UI
{
    public class ConsoleDownloadProgressBar
    {
        ...
        public void Update(FileObject file, float pourcentage, long bytesReceived, long totalBytes)
        {
            ...
            SetText("File Size   : " + AlignText(HumanReadableByteCount(totalBytes), 50, false), ConsoleColor.Yellow, 0,
                    y + 3);
            SetText("Downloaded  : " + AlignText(HumanReadableByteCount(bytesReceived), 50, false), ConsoleColor.Yellow,
                    0, y + 4);
            SetText("Pourcent    : " + AlignText(pourcentage.ToString(CultureInfo.InvariantCulture) + "%", 50, false),
